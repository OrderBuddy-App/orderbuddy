...
-- ===========================================================================
-- V31.8 – Veranstaltungspositionen: Servierfolge, Bemerkungen, Extras/Optionen
-- ===========================================================================
begin;
alter table public.event_items add column if not exists course text not null default 'main';
alter table public.event_items add column if not exists note text;
alter table public.event_items add column if not exists options jsonb not null default '{}'::jsonb;
alter table public.event_items add column if not exists extras jsonb not null default '[]'::jsonb;
alter table public.event_items drop constraint if exists event_items_course_check;
alter table public.event_items add constraint event_items_course_check check (course in ('main','starter','together'));
commit;

-- ===========================================================================
-- V31.9 – Veranstaltungsstatus wie Tischbetrieb + Service-Update
-- ===========================================================================
begin;

alter table public.events add column if not exists package_status text not null default 'new';
alter table public.events add column if not exists ready_for_checkout boolean not null default false;
alter table public.events drop constraint if exists events_package_status_check;
alter table public.events add constraint events_package_status_check check (package_status in ('new','preparing','done'));
...
alter table public.event_items add constraint event_items_status_check check (status in ('new','preparing','done'));

drop policy if exists orderbuddy_event_items_update on public.event_items;
create policy orderbuddy_event_items_update on public.event_items
for update to authenticated
using (
  public.user_has_restaurant_access(restaurant_id)
  and exists (
...
-- Abschluss serverseitig ebenfalls nur nach bewusstem Grün erlauben.
create or replace function public.close_orderbuddy_event_with_payments(p_event_id uuid, p_payments jsonb)
returns jsonb
language plpgsql
security definer
